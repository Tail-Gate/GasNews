...
class WrapSerializer:
    ...
    def convert_to_utc(value: Any, handler, info) -> Dict[str, datetime]:
        ...
        if info.mode == 'json':
            return {
                k: datetime.fromisoformat(v).astimezone(datetime.UTC)
                for k, v in partial_result.items()
            }
        return {k: v.astimezone(datetime.UTC) for k, v in partial_result.items()}
    ...
    '''
    {
        'event_datetime': {
            'start': datetime.datetime(
                2024, 1, 1, 15, 0, tzinfo=datetime.datetime.UTC
            ),
            'end': datetime.datetime(
                2024, 1, 3, 14, 0, tzinfo=datetime.datetime.UTC
            ),
        }
    }
    '''
